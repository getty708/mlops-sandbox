...
"""Demo Script for running segmentation model on images and saving the results.
This is copied from the original github repo.

Reference:
- https://github.com/facebookresearch/sapiens/blob/main/lite/demo/vis_seg.py
...
from services.segmentation.sapiens.external.adhoc_image_dataset import AdhocImageDataset
from services.segmentation.sapiens.external.classes_and_palettes import (
    GOLIATH_CLASSES,
    GOLIATH_PALETTE,
)
from services.segmentation.sapiens.external.worker_pool import WorkerPool
...
def img_save_and_viz(
    ...
    output_file = (
        output_path.replace(".jpg", ".png").replace(".jpeg", ".png").replace(".png", ".npy")
    )
    output_seg_file = (
        output_path.replace(".jpg", ".png").replace(".jpeg", ".png").replace(".png", "_seg.npy")
    )

    image = image.data.numpy()  ## bgr image

    seg_logits = F.interpolate(result.unsqueeze(0), size=image.shape[:2], mode="bilinear").squeeze(
        0
    )
